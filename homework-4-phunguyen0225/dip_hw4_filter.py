...
def main():
    ...
    if noise == 'bipolar':
        ...
        noisy_image, var = get_bipolar_noise(
            input_image, noise_proba, noise_probb)
    ...
    if filter == 'local_noise':
        Filter_obj = Filtering(np.array(noisy_image),
                               filter, filter_size, var=var)
    ...
    output_image_name = output_dir + image_name + \
        "_denoised_" + filter + "_" + noise_name + ".jpg"
